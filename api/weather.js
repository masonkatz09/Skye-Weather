export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'alerts') {
      ...
      if (isCA) {
        const bb =
          (la>43.4&&la<44.2&&lo>-80.0&&lo<-78.8) ? 'on61' :
          ...
          (la>46&&la<61&&lo>-68&&lo<-52)             ? 'nl33' : 'on61';
...
').trim();
    ...
    if (type === 'postal') {
      ...
      if (isCA) {
        ...
        // Precise FSA centroid coordinates for Canadian neighbourhoods
        const FSA_COORDS = {
          ...
          M5A:{lat:43.6543,lon:-79.3607,name:'Regent Park',prov:'ON'},M5B:{lat:43.6572,lon:-79.3783,name:'Garden District',prov:'ON'},M5C:{lat:43.6517,lon:-79.3752,name:'St. James Town',prov:'ON'},M5E:{lat:43.6441,lon:-79.3733,name:'Berczy Park',prov:'ON'},M5G:{lat:43.6579,lon:-79.3879,name:'Central Bay Street',prov:'ON'},M5H:{lat:43.6489,lon:-79.3825,name:'Richmond',prov:'ON'},M5J:{lat:43.6401,lon:-79.3812,name:'Harbourfront East',prov:'ON'},M5K:{lat:43.6472,lon:-79.3813,name:'Toronto Islands',prov:'ON'},M5L:{lat:43.6481,lon:-79.3793,name:'Commerce Court',prov:'ON'},M5M:{lat:43.7327,lon:-79.4228,name:'Bedford Park',prov:'ON'},M5N:{lat:43.7175,lon:-79.4193,name:'Roselawn',prov:'ON'},M5P:{lat:43.6966,lon:-79.4107,name:'Forest Hill North',prov:'ON'},M5R:{lat:43.6727,lon:-79.4109,name:'The Annex',prov:'ON'},M5S:{lat:43.6626,lon:-79.4000,name:'University',prov:'ON'},M5T:{lat:43.6538,lon:-79.4012,name:'Kensington Market',prov:'ON'},M5V:{lat:43.6289,lon:-79.3958,name:'CN Tower/King West',prov:'ON'},M5X:{lat:43.6484,lon:-79.3828,name:'First Canadian Place',prov:'ON'},
        ...
        const data = FSA_COORDS[fsa] || FSA_COORDS[fsa.slice(0,2)] || FSA_COORDS[fsa[0]];
        if (data) {
          return res.json([{
